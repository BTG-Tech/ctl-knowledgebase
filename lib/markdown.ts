...
export async function getAllBlogStaticPaths() {
  try {
    const blogFolder = path.join(process.cwd(), "/contents/resources/");
...
export async function getAllBlogs() {
  ...
  const uncheckedRes = await Promise.all(
    files.map(async (file) => {
      ...
      const filepath = path.join(process.cwd(), `/contents/resources/${file}`);
...
export async function getBlogForSlug(slug: string) {
  const blogFile = path.join(process.cwd(), "/contents/resources/", `${slug}.mdx`);
  try {
